...
void assert_cdfs(const std::vector<std::vector<int>> &cdfs,
                ...
                py::array_t<int32_t> &cdf_sizes){
  auto buf_sizes = cdf_sizes.request();
  int32_t* ptr_sizes = (int32_t*) buf_sizes.ptr;
  for (int i = 0; i < static_cast<int>(cdfs.size()); ++i) {
    assert(cdfs[i][0] == 0);
    assert(cdfs[i][ptr_sizes[i] - 1] == (1 << precision));
    for (int j = 0; j < ptr_sizes[i] - 1; ++j) {
...
RansDecoder::decode_with_indexes(const std::string &encoded,
                                 ...
                                 py::array_t<int32_t> &cdfs_sizes,
                                 py::array_t<int32_t> &offsets) {

  auto buf_indexes = indexes.request(), buf_sizes = cdfs_sizes.request(), buf_offsets = offsets.request();
  //clock_t t1 = clock();
  //assert(cdfs.size() == cdfs_sizes.size());
  assert(cdfs.size() == buf_sizes.shape()[0]);
  assert_cdfs(cdfs, cdfs_sizes);

  //std::vector<int32_t> output(indexes.shape[0];

  auto output = py::array_t<int32_t>(buf_indexes.size);
  py::buffer_info buf_output = output.request();
  //   nullptr,            /* Pointer to data (nullptr -> ask NumPy to allocate!) */
  //   sizeof(int32_t),     /* Size of one item */
  //   py::format_descriptor<int32_t>::value, /* Buffer format */
  //   buf_indexes.ndim,          /* How many dimensions? */
  //   { indexes.shape()[0] },  /* Number of elements for each dimension */
  //   { sizeof(int32_t) }  /* Strides for each dimension */
  //  ));

   int32_t* ptr_indexes = (int32_t*)buf_indexes.ptr;
   int32_t* ptr_sizes = (int32_t*)buf_sizes.ptr;
   int32_t* ptr_offsets = (int32_t*)buf_offsets.ptr;
   int32_t* ptr_output = (int32_t*)buf_output.ptr;
  ...
  for (int i = 0; i < static_cast<int>(indexes.shape()[0]); ++i) {
    const int32_t cdf_idx = ptr_indexes[i];
    assert(cdf_idx >= 0);
    assert(cdf_idx < cdfs.size());

    const auto &cdf = cdfs[cdf_idx];

    const int32_t max_value = ptr_sizes[cdf_idx] - 2;
    assert(max_value >= 0);
    assert((max_value + 1) < cdf.size());

    const int32_t offset = ptr_offsets[cdf_idx];

    const uint32_t cum_freq = Rans64DecGet(&rans, precision);

    const auto cdf_end = cdf.begin() + ptr_sizes[cdf_idx];
    ...
    ptr_output[i] = value + offset;
  }
  //clock_t t2 = clock();
  //std::cout << "decoding time >> " << (double)(t2 - t1) / CLOCKS_PER_SEC << std::endl;
  return output;
}
...
PYBIND11_MODULE(ans, m) {
  ...
  py::class_<BufferedRansEncoder>(m, "BufferedRansEncoder")
      .def(py::init<>())
      .def("encode_with_indexes", &BufferedRansEncoder::encode_with_indexes, py::call_guard<py::scoped_ostream_redirect,
