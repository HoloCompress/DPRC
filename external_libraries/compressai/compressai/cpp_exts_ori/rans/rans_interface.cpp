...
//#pragma omp parallel for
  for (size_t i = 0; i < symbols.size(); ++i) {
    ...
    _syms.at(num_symbols++) = {
        static_cast<uint16_t>(cdf[value]),
        static_cast<uint16_t>(cdf[value + 1] - cdf[value]), false};
    ...
    if (value == max_value) {
      ...
      while (val >= max_bypass_val) {
        ...
        _syms[num_symbols++] = {max_bypass_val, max_bypass_val + 1, true};
        val -= max_bypass_val;
      }
      _syms[num_symbols++] = {static_cast<uint16_t>(val),
...
py::bytes BufferedRansEncoder::flush() {
  ...
  std::vector<uint32_t> output(num_symbols, 0xCC);
...
RansEncoder::encode_with_indexes(const std::vector<int32_t> &symbols,
  ...
  buffered_rans_enc.encode_with_indexes(symbols, indexes, cdfs, cdfs_sizes,
                                        offsets);
  clock_t t2 = clock();
  std::cout << "encoding time >> " << (double)(t2 - t1) / CLOCKS_PER_SEC << std::endl;
