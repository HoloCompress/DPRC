...
class EntropyModel(nn.Module):
    ...
    def compress(self, inputs, indexes, means=None):
        ...
        print('++++++++++++++++in entropy model compression++++++++++++++')
        print('type of cdf_length is ', type(self._cdf_length), self._cdf_length.shape)
        print('type of _offset is  ', type(self._offset), self._offset.shape)
        print('type of _quantized_cdf  is ', type(self._quantized_cdf), self._quantized_cdf.shape)
        print('shape of symbols is ', symbols.shape, symbols.device)
        print('shape of indexes is ', indexes.shape, indexes.device)
        print('first elements in _quantized_cdf...', self._quantized_cdf[0])
        print('++++++++++++++++end entropy model compression++++++++++++++')

        strings = []
        
        # for i in range(symbols.size(0)):
        cur_symbols = symbols.reshape(-1).int().tolist()
        cur_indexes = indexes.reshape(-1).int().tolist()
        quantized_cdf = self._quantized_cdf.tolist()
        cdf_length = self._cdf_length.reshape(-1).tolist()
        offset = self._offset.reshape(-1).tolist()
        rv = self.entropy_coder.encode_with_indexes(cur_symbols, cur_indexes, quantized_cdf, cdf_length, offset)
        strings.append(rv)
        return strings
