...
DATASETS_DICT = {"openimages": "OpenImages", "cityscapes": "CityScapes", 
                 "jetimages": "JetImages", "evaluation": "Evaluation", "div2k":"DIV2K"}
...
class DIV2K(BaseDataset):
    ...
    def __init__(self, root='/mnt/mnt2/DIV2K/DIV2K', mode='train', crop_size=512, 
        normalize=False, **kwargs):
        super().__init__(root, [transforms.ToTensor()], **kwargs)

        if mode == 'train':
            #data_dir = self.train_data
            data_dir = '/mnt/mnt2/DIV2K/DIV2K_train_HR_patch'
        elif mode == 'validation':
            #data_dir = self.val_data
            data_dir = '/mnt/mnt2/DIV2K/DIV2K_valid_HR_patch'
        else:
            raise ValueError('Unknown mode!')
        ...
        #self.imgs = glob.glob(os.path.join(data_dir, '*.jpg'))
        #self.imgs += glob.glob(os.path.join(data_dir, '*.png'))
        self.imgs = glob.glob(os.path.join(data_dir, '*full-holo-rgb-512.png'))
