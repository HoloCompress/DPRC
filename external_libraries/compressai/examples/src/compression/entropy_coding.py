...
def _indexed_cdf_to_enc_statfun(cdf_i):
    ...
    def _enc_statfun(value):
        ...
        lower = cdf_i[value]
        print('within indexed_cdf_to_enc  type of position lower and lower+ is ')
        print(type(lower))
        print(type(cdf_i[int(value+ np.int64(1))]))
        # cum_freq, pmf @ value
...
def ans_index_encoder(symbols, indices, cdf, cdf_length, cdf_offset, precision, 
    ...
    # print('type of instruction ', instructions[0].detype)
    # print('type of instruction ', type(instructions[1][0]))
...
# def handle_single_patch(data, precision, overflow_width,
#                                 enc_statfun_overflow, dec_statfun_overflow):
#     # # Bin of discrete CDF that value belongs to
#     # value_i = values[i]
#     # cdf_index_i = cdf_index[i]        
#     # cdf_i = cdf[cdf_index_i]
#     # #print('cdf_index_i is...', cdf_index_i)
#     # cdf_length_i = cdf_length[cdf_index_i]
#     #print('cdf_length_i is ...', cdf_length_i)
#     #print(value_i.shape, cdf_i.shape, cdf_index_i.shape)
#     value_i, cdf_index_i, cdf_i, cdf_length_i, overflow_i  = data
#     results = []
...
#     results.append((start, freq, False, precision, 0))
...
#             #print('cond_mask is true')
#             widths = np.where(cond_mask, widths+1, widths)
#             cond_mask = (overflow_i >> (widths * overflow_width)) != 0
            
#         val = widths
#         cond_mask = val >= max_overflow
#         #print('cond mask after operations, ', np.sum(cond_mask))
#         while np.any(cond_mask):    
#             #print('np.sum(cond_mask) is ', np.sum(cond_mask))
...
#             results.append((start, freq, True, int(overflow_width), of_mask))
...
def vec_ans_index_buffered_encoder(symbols, indices, cdf, cdf_length, cdf_offset, precision, 
    coding_shape, overflow_width=OVERFLOW_WIDTH, patch_size=PATCH_SIZE, **kwargs):
    """
    ...
    if B == 1:
        ...
        values, _ = compression_utils.decompose(values, n_channels, patch_size)
        overflow, _ = compression_utils.decompose(overflow, n_channels, patch_size)
        cdf_index, unfolded_shape = compression_utils.decompose(indices, n_channels, patch_size)
        coding_shape = values.shape[1:]
        assert coding_shape == cdf_index.shape[1:]

    # print('patch size is ', patch_size)
    # # LIFO - last item in buffer is first item decompressed
    # print('len(cdf_index) is ', len(cdf_index))
    # print('after decompose, shape of values is ', values.shape)
    # print('after decompose, shape of overflow is ', overflow.shape)
    # print('after decompose, shape of cdf_index is ', cdf_index.shape)
    # print('len(cdf_index is ', len(cdf_index))  #len(cdf_index is  128640

    #print('directly read ', values[0].shape)
    '''
    Attempt to use multiprocessing to accelerate, but there is no apparent improvement

    value_list = np.split(values, values.shape[0], axis=0)
    overflow_list = np.split(overflow, overflow.shape[0], axis=0)
    cdf_index_list = np.split(cdf_index, cdf_index.shape[0], axis=0)
    value_list = [item[0,...] for item in value_list]
    overflow_list = [item[0,...] for item in overflow_list]
    cdf_index_list = [item[0,...] for item in cdf_index_list]
    cdf_list = [cdf[item] for item in cdf_index_list]
    cdf_len_list = [cdf_length[item] for item in cdf_index_list]
    data_list = zip(value_list, cdf_index_list, cdf_list, cdf_len_list, overflow_list)

    pool = ThreadPool(4)


    kwargs = {'precision': precision, 'overflow_width': overflow_width,
               'enc_statfun_overflow': enc_statfun_overflow, 'dec_statfun_overflow':dec_statfun_overflow}
    handle_patch_func = partial(handle_single_patch, **kwargs)
    all_results = pool.map(handle_patch_func, data_list)
    instructions = list(chain(*all_results))
    # instructions = list(chain(*all_results))
    '''
    max_overflow = (1 << overflow_width) - 1
...
def vec_ans_index_encoder(symbols, indices, cdf, cdf_length, cdf_offset, precision, 
    ...
    #print('++++++++++++++++++++ in vec_ans_index_encoder ++++++++++++++++++++++++++++++')
    start = time.time()
    instructions, coding_shape = vec_ans_index_buffered_encoder(symbols, indices, cdf,
        cdf_length, cdf_offset, precision, coding_shape, overflow_width, patch_size)
    #print('first step elpsed time .... ', time.time() - start)
    start = time.time()
    encoded = vec_ans_index_encoder_flush(instructions, precision, coding_shape, overflow_width)
    #print('second step elapsed time ... ', time.time() - start)
    return encoded, coding_shape
...
def vec_ans_index_decoder(encoded, indices, cdf, cdf_length, cdf_offset, precision, 
    ...
    if B == 1:
        decoded = compression_utils.reconstitute(np.stack(symbols, axis=0), padded_shape, unfolded_shape, patch_size=patch_size)
