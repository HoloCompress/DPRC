...
def ans_compress(symbols, indices, cdf, cdf_length, cdf_offset, coding_shape,
    precision, vectorize=False, block_encode=True, patch_size=PATCH_SIZE):
    # print('in ans compress +++++++++++++++++++++++++++')
    # print('coding shape is ', coding_shape) #(320, 67, 120)
    # print('precision is ', precision) #16
    # print('cdf_length is ', cdf_length)
    # print('cdf offset length is ',len(cdf_offset)) #320
    ...
    # print('type of encoded elements:  ')
    # print(type(encoded[0][0]))
    # print(type(encoded[1][0]))
    # print('+++++++++++++++++++++++++++++++++++ end +++++++++++++++++++++++++++++++')
    return encoded


def ans_decompress(encoded, indices, cdf, cdf_length, cdf_offset, coding_shape,
    precision, vectorize=False, block_decode=True, patch_size=PATCH_SIZE):
...
def write_bytes_message(f_out, message):
    f_out.write(message)
...
def save_compressed_format(compression_output, out_path):
    ...
    with open(out_path, 'wb') as f_out:
        # print('the shape of hyperlent is ', compression_output.hyperlatents_encoded.shape)
        # print('the shape of latent encoded is ', compression_output.latents_encoded.shape)
        ...
        #write_num_bytes_encoded(len(enc_hyperlatents), f_out)
        #message_to_bytes(f_out, enc_hyperlatents)
        write_bytes_message(f_out, enc_hyperlatents)
        f_out.write(_MAGIC_VALUE_SEP)
        #print(enc_hyperlatents.type())
        # print('enc_hyperlatents type is ', enc_hyperlatents[:4])
        ...
        write_bytes_message(f_out, enc_latents)
        f_out.write(_MAGIC_VALUE_SEP)
        # print('enc_latents type is ', enc_latents[:4])
        '''
        # with open('saved_message.npy', 'wb') as f:  
            np.save(f, enc_hyperlatents)
            np.save(f, enc_latents)
        '''
...
def load_compressed_format(in_path):
    ...
    with open(in_path, 'rb') as f_in:
        ...
        assert f_in.read(4) == _MAGIC_VALUE_SEP  # assert valid file
        #print('first 4 elems in hyperlatents encod is ', hyperlatents_encoded[:4])

        # Read latents
        num_bytes = read_num_bytes_encoded(f_in)
        latents_encoded = f_in.read(num_bytes)
        
        #lentes_encoded = read_bytes(f_in, num_bytes)
        #print('first 4 elems in latents encod is ', latents_encoded[:4])
        assert f_in.read(4) == _MAGIC_VALUE_SEP  # assert valid file
