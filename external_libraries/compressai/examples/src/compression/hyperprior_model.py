...
class HyperpriorEntropyModel(entropy_models.ContinuousEntropyModel):
    ...
    def compress(self, bottleneck, block_encode=True, vectorize=False):
        ...
        indice_time = time.time() - start 
        # print('bulding indices takes %.4f'%(indice_time))

        # print('in hyper_prior model, the shape of bottleneck is  ', bottleneck.shape)
        # print('in hyper prior model, the shape of indices is ', indices.shape)
        ...
        # print('move symbols to cpu takes ...', time.time() - start)
        ...
        # print('within hyperprior model, before ans compress, the elapsed time is ', elapsed)
        # print('within hyperprior_model, vectorize is ', vectorize)

        '''
        for hyper latents
        in hyper_prior model, the shape of bottleneck is   torch.Size([1, 320, 67, 120])
        in hyper prior model, the shape of indices is  torch.Size([320, 67, 120])
        within hyperprior model, before ans compress, the elapsed time is  0.1483316421508789
        within hyperprior_model, vectorize is  True
        '''
    ...
    def decompress(self, encoded, batch_shape, broadcast_shape, coding_shape, vectorize=False, 
        ...
        # cdf_length = cdf_length.reshape(-1).int().tolist()
        # cdf_offset = cdf_offset.reshape(-1).int().tolist()

        prepare_time = time.time() - start 
        print('in hyperprior , data prepare time is ', prepare_time)
        new_start = time.time()
        ...
        decode_eplase = time.time() - new_start
        print('in hyperprior, decoding time is ', decode_eplase)

        transfer_start = time.time()
        #symbols = torch.from_numpy(toarr(decoded))
