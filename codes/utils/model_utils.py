...
class Vgg19_conv4(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        for x in range(21, 30):
            self.conv5.add_module(str(x), vgg_pretrained_features[x])
        if not requires_grad:
            for param in self.parameters():
                param.requires_grad = False

    def forward(self, X):
        h_relu1 = self.conv1(X)
        h_relu2 = self.conv2(h_relu1)        
        h_relu3 = self.conv3(h_relu2)        
        h_relu4 = self.conv4(h_relu3)        
        #h_relu5 = self.conv5(h_relu4)                
        out = h_relu4
        return out