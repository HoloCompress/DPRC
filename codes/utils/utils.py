"""
This file provides uitlity funtions and is borrowed from the implementation of the following paper:
...
def write_sgd_summary(slm_phase, out_amp, target_amp, k,
    ...
    target_amp = target_amp.squeeze()
    out_amp = out_amp.squeeze()
    print(target_amp.size())
    print(out_amp.size())
    if target_amp.size(0) == 3 and target_amp.size(0) == out_amp.size(0):
        target_amp = target_amp.permute(1,2,0)
        out_amp = out_amp.permute(1,2,0)
    psnr_value = psnr(target_amp.cpu().detach().numpy(), (s * out_amp).cpu().detach().numpy())
    ssim_value = ssim(target_amp.cpu().detach().numpy(), (s * out_amp).cpu().detach().numpy(), multichannel=True)

    if writer is not None:
        writer.add_image(f'{prefix}_Recon/amp', (s * out_amp).squeeze(0), k, dataformats='HWC')
