...
from src.compression import hyperprior_model, prior_model, compression_utils
...
class Hyperprior(CodingModel):
    ...
    def __init__(self, bottleneck_capacity=220, hyperlatent_filters=LARGE_HYPERLATENT_FILTERS, 
        ...
        vectorize_encoding=True, block_encode=True, is_train=True):
        ...
        self.synthesis_stats = synthesis_net(C=int(2*bottleneck_capacity), N=hyperlatent_filters)
        
        self.amortization_models = [self.analysis_net, self.synthesis_stats]
        ...
        if entropy_code is True:
            ...
            self.prior_density = prior_model.PriorDensity(n_channels= int(bottleneck_capacity * 16),
    ...
    def reshape(self, input_tensor):
        ...
        input_tensor = input_tensor.view(B, C,  4, H // 4, 4, W // 4)
        input_tensor = input_tensor.permute(0, 1, 2, 4, 3 , 5).contiguous().view(B, int(C * 16), H//4, W//4)
        return input_tensor

    def reshape_back(self, input_tensor):
        B, C, H, W = input_tensor.shape 
        input_tensor = input_tensor.view(B, C // 16, 16,  H, W).view(B, C // 16, 4, 4, H, W)
        input_tensor = input_tensor.permute(0, 1, 2, 4, 3 ,5 ).contiguous().view(B, C//16, int(H * 4), int(W * 4))
        return input_tensor


    def compress_forward(self, latents, spatial_shape, **kwargs):
        # print('++++++++++++++ in compress forward +++++++++++++++++++++++')
        start = time.time()
        ...
        # print('hyperlatents shape is before encoing ', hyperlatents.shape)
        ...
        # print('estimate compress bits takes ... ', time.time() - start)
        ...
        #print('hyperlatents shape after encoing ', hyperlatents_encoded.shape)
        # print('encoding hyperlatents takes....', time.time() - start)
        # # hyper_decode_start = time.time()
        ...
        # # print('hyperlatents decoding takes...', time.time() - hyper_decode_start)
        
        # # print('hyperlatents shape after decoding ', hyperlatents_decoded.shape)
        ...
        latent_stats = self.synthesis_stats(quantized_hyperlatents)
        #latent_stats = self.synthesis_stats(hyperlatents_decoded)
        latent_means, latent_scales = torch.split(latent_stats, self.bottleneck_capacity, dim=1)
        ...
        # print('latents shape after decoding ', latents.shape)


        latent_means = self.reshape(latent_means)
        latent_scales = self.reshape(latent_scales)
        latents = self.reshape(latents)
        # print('latents_means.shape', latent_means.shape)
        # print('latents scales.shape', latent_scales.shape)

        latents_encoded, latent_coding_shape, _ = self.prior_entropy_model.compress(latents, means=latent_means,
            scales=latent_scales, vectorize=self.vectorize_encoding, block_encode=self.block_encode, patch_size=(1,1))
        # print('compress latents takes ... ', time.time() - latents_start)
        #print('latents shape after encoding ', latents_encoded.shape)
        #print('latents_coding_shape is ', latent_coding_shape)
        
        # print('within compress process, the first 4 elems in hyper latents is ',  hyperlatents_encoded[:4])
        # print('within compress process, the first 4 elems in latents is ',  latents_encoded[:4])
        ...
        # with open('saved_message.npy', 'wb') as f:
        #     np.save(f, hyperlatents_encoded)
        #     np.save(f, latents_encoded)

        # with open('saved_message.npy', 'rb') as f: 
        #     hyperlatents_encoded_load = np.load(f)
        #     latents_encoded_load = np.load(f)
        #     diff_hyper = hyperlatents_encoded_load - hyperlatents_encoded
        #     diff_hyper = np.sum(diff_hyper) /  hyperlatents_encoded.shape[0]
        #     print('diff hyper is ', diff_hyper)
        #     diff_latent = latents_encoded_load - latents_encoded 
        #     diff_latent = np.sum(diff_latent) / (latents_encoded.shape[0])
        #     print('average diff latents is ', diff_latent)
        #     print('load from saved message')

        # comp_path = '/mnt/mnt2/codes/yujie_codes/holonet_compression/holonet_latent2x_quantization_finetune_lr1e-4_Negssim/train_log/results/test-ckpt-latest-compress/0801-compressed.hfc'
        # actual_bpp, theoretical_bpp = compression_utils.save_compressed_format(compression_output,
        #         out_path=comp_path)
                
        # compression_output_load = compression_utils.load_compressed_format(comp_path)
        # load_latents = compression_output_load.latents_encoded
        # load_hyper = compression_output_load.hyperlatents_encoded
        # diff_hyper = np.sum(load_hyper - hyperlatents_encoded) /  hyperlatents_encoded.shape[0]
        # print(latents_encoded.shape)
        # diff_latent = np.sum(load_latents - latents_encoded) / latents_encoded.shape[0]
        # print('average diff latents is ', diff_latent)
        # print('average diff hyper is ', diff_hyper)

        return compression_output

    def decompress_forward(self, compression_output, device):

        # print('++++++++++++++++++++++in decompress forward+++++++++++++++++++++++++++')
        start = time.time()
        hyperlatents_encoded = compression_output.hyperlatents_encoded
        latents_encoded = compression_output.latents_encoded
        # with open('saved_message.npy', 'rb') as f: 
        #     hyperlatents_encoded = np.load(f)
        #     latents_encoded = np.load(f)
        #     print('load from saved message')
        ...
        hyperlatents_decoded = self.hyperprior_entropy_model.decompress(hyperlatents_encoded,
            batch_shape=batch_shape, broadcast_shape=hyperlatent_spatial_shape,
        ...
        # print('decode hyperlatents takes .... ', time.time() - start )
        ...
        latent_means = self.reshape(latent_means)
        latent_scales = self.reshape(latent_scales)
        print("hyper prior decoder time is ", time.time() - latents_start)
        ...
        latents_decoded = self.prior_entropy_model.decompress(latents_encoded, means=latent_means,
            ...
            block_decode=self.block_encode, patch_size=(1,1))

        reshape_start = time.time()
        latents_decoded = self.reshape_back(latents_decoded)
        # print('reshape takes ... ', time.time() - reshape_start)
        # print('after decoding, latent_decoded shape is ', latents_decoded.shape)
        # print('decode latents takes.... ', time.time() - latents_start)
        latents = latents_decoded.to(device)
        return latents
...
class Hyperprior_new(CodingModel):
    ...
    def decompress_forward(self, compression_output, device):
        ...
        # Recover latent statistics from compressed hyperlatents
        start = time.time()
        latent_stats = self.synthesis_mu_std(hyperlatents_decoded)
        #latent_scales = self.synthesis_std(hyperlatents_decoded)
        latent_means, latent_scales = torch.split(latent_stats, 4, dim=1)
        latent_scales = lower_bound_toward(latent_scales, self.scale_lower_bound)
        latent_spatial_shape = latent_scales.size()[2:]
        print('hyper decoder time from is ', time.time() - start)
