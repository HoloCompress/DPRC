...
class PriorEntropyModel(entropy_models.ContinuousEntropyModel):
    ...
    def __init__(self, distribution, scale_table=None, index_ranges=64, min_scale=MIN_SCALE,
        ...
        '''
        test compress ai compress and decompress 
        '''
    ...
    def compress(self, bottleneck, means, scales, vectorize=False, block_encode=True, patch_size=(1,1)):
        """
        ...
        # print('+++++++++++++ in prior compress ++++++++++++++++++++++++')
        start = time.time()

        indices = self.compute_indices(scales)
        # print('bulding indices takes ... ', time.time() - start )
        ...
        for i in range(symbols.size(0)):
            rv = self.encoder.encode_with_indexes(
                symbols[i].reshape(-1).int().tolist(), 
                indices[i].reshape(-1).int().tolist(),
                cdf.tolist(),
                cdf_length.reshape(-1).int().tolist(),
                cdf_offset.reshape(-1).int().tolist(),
            )
            strings.append(rv)
        strings = strings[0]
        # print('encoding takes....', time.time() - start)
        ...
        #     block_encode=block_encode, patch_size=patch_size)
        # elapsed = time.time() - start 
        # print('encoding latents time is ... ', elapsed)
        # print('++++++++++++end  prior compress +++++++++++++++++++++++++++++')

        # return encoded, coding_shape, rounded
        return strings, coding_shape, None

    
    def decompress(self, encoded, means, scales, broadcast_shape, coding_shape, vectorize=False, 
        block_decode=True, patch_size=(1,1)):
        """
        ...
        indices = self.compute_indices(scales)
        # print('symbol shape and indice shape are, ' , symbols_shape, indices.shape)
        ...
        # cdf_length = self.CDF_length.cpu().numpy()
        # cdf_offset = self.CDF_offset.cpu().numpy()
        # outputs = cdf.new(indices.size())
        cdf_length = self.CDF_length 
        cdf_offset = self.CDF_offset 
        cdf = self.CDF
        prepare_time = time.time() - start 
        #print('in prior model, prepare time is ', prepare_time)
       
        #indices = indices.reshape(-1).int().tolist()
        #cdf_np = cdf.cpu().numpy()
        indices = indices.reshape(-1).int().cpu().numpy()
        #print('in prior model, cdf np shape is ', cdf_np.shape)
        tolist_start = time.time()
        cdf = cdf.tolist()
        #print('in prior model, to list time is ', time.time()-tolist_start)
        #print('in prior model, cdf shape is ', len(cdf), len(cdf[0]), len(cdf[1]))
        cdf_length = cdf_length.reshape(-1).int().cpu().numpy()
        #print('in prior model, cdf length is ', cdf_length.shape, cdf_length[0])
        #cdf_length = cdf_length.int().tolist()
        #cdf_offset = cdf_offset.reshape(-1).int().tolist()
        cdf_offset = cdf_offset.reshape(-1).int().cpu().numpy()
        
        decode_start = time.time()
        decoded = self.decoder.decode_with_indexes(encoded,
                                            indices,
                                            cdf,
                                            cdf_length,
                                            cdf_offset)
        #     
        # decoded = self.decoder.decode_with_indexes(encoded,
        #                                     indices.reshape(-1).int().tolist(),
        #                                     cdf.tolist(),
        #                                     cdf_length.reshape(-1).int().tolist(),
        #                                     cdf_offset.reshape(-1).tolist())
        #print('in prior model, decoding time is ', time.time() - decode_start)
        # decoded = compression_utils.ans_decompress(encoded, indices, cdf, cdf_length, cdf_offset,
        #     coding_shape, precision=self.precision, vectorize=vectorize, block_decode=block_decode, patch_size=patch_size)
        transfer_start = time.time()
        #decoded_np = toarr(decoded)
        symbols = torch.from_numpy(decoded)
        #print('to arr costs time ', time.time()-transfer_start)
        
        
        # transfer_start = time.time()
        # symbols = torch.Tensor(decoded)
        # print('in prior model. after decoding, constructing symbol tensor costs ', time.time()-transfer_start)
        symbols = torch.reshape(symbols, symbols_shape)
        #decoded_raw = symbols.clone()
        decoded = self.dequantize(symbols, offsets=means)
        #print('in prior model, after decoding, transfer time is ' , time.time() - transfer_start)
