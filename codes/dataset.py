...
def get_dataloader(phase, config):
    ...
    is_vertical = phase == 'train'
    is_horizontal = phase == 'train'
    input_paths = []
    ...
    if config.dataset == 'DIV2K': 
        if phase == 'train':
            data_path = os.path.join(config.holo_data_root, f'{config.dataset}_train_HR')
        else:
            data_path = os.path.join(config.holo_data_root, f'{config.dataset}_valid_HR')
    else: 
        data_path = os.path.join(config.holo_data_root, f'{config.dataset}')
    ...
    dataloader = ImageLoader(data_path, channel=channels[config.channel],
                           ...
                           crop_to_homography=True, shuffle=is_shuffle, 
                           vertical_flips=is_vertical, horizontal_flips=is_horizontal)
...
class ImageLoader:
    ...
    def load_batch(self, images):
        ...
        ims = torch.stack([im for im, _, _, _ in im_res_name], 0)
        masks = torch.stack([mask for _, mask, _, _ in im_res_name],0)
        return (ims,
                masks,
                [res for _, _, res, _ in im_res_name],
                [name for _, _, _, name in im_res_name])
...
def get_image_filenames(dir):
    ...
    image_types = ['png', 'bmp', 'gif']
    if 'collected' in dir: 
        image_types.extend(['jpg', 'jpeg'])
...
if __name__ == "__main__":
    ...
    class config:
        def __init__(self, data_root=None):
            ...
            self.holo_data_root = '/home/yujie/yujie_data/DIV2K'
            self.batch_size=2
            self.channel = 'r'
    ...
    # get target image
        target_amp, mask, target_res, target_filename = target
        print(target_amp.shape)
        if k == 0:
            import skimage.io as sio
            target_amp = target_amp[0].numpy().squeeze()
            mask = mask[0].numpy().squeeze()
            sio.imsave('test_amp_save.png',target_amp)
            sio.imsave('test_mask_save.png', mask)